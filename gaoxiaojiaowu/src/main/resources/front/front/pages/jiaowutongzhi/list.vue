<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.jiaowutongzhiName" type="text" placeholder="标题"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in jiaowutongzhiTypesList" :key="index"
...
<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
    <view style="margin: 10rpx 10rpx 10rpx 10rpx;"
        ...
        <image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill" class="listmpic"
               :src="product.jiaowutongzhiPhoto?product.jiaowutongzhiPhoto.split(',')[0]:''"></image>
        <view class="listmr">
            <view class="col3 f30 elip mb15"
                  :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"rgba(255, 255, 255, 1)","textAlign":"center"}'>
                {{product.jiaowutongzhiName}}
            </view>
        </view>
           <view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
            <text style="margin-left: 50rpx;" v-if="isAuth('jiaowutongzhi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">
                修改
            </text>
            <text v-if="isAuth('jiaowutongzhi','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('jiaowutongzhi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 类型 字段 字典表查询方法
		  	let jiaowutongzhiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'jiaowutongzhi_types'
		  	});
		  	this.jiaowutongzhiTypesList = jiaowutongzhiTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.jiaowutongzhiName = "";
			},
			//类别搜索
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.jiaowutongzhiTypes = this.categoryName
                            }

                let res = await this.$api.list(`jiaowutongzhi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiaowutongzhi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.jiaowutongzhiName) {
                    searchForm['jiaowutongzhiName'] = this.searchForm.jiaowutongzhiName
                }
				if (this.searchForm.jiaowutongzhiName) {
					searchForm['jiaowutongzhiName'] = this.searchForm.jiaowutongzhiName
				}

                let res = await this.$api.list(`jiaowutongzhi`, searchForm);
				// 如果是第一页数据置空
