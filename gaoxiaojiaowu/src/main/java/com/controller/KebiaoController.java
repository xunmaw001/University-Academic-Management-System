    ...
    */
    ...
    public R save(@RequestBody KebiaoEntity kebiao, HttpServletRequest request){
        ...
        if(kebiaoEntity==null){
            ...
            return R.error(511,"每个班级只能添加一张课表");
        }
    }
...
//            return R.error(511,"权限为空");
        ...
        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        KebiaoEntity kebiaoEntity = kebiaoService.selectOne(queryWrapper);
        if("".equals(kebiao.getKebiaoPhoto()) || "null".equals(kebiao.getKebiaoPhoto())){
                kebiao.setKebiaoPhoto(null);
        }
        if(kebiaoEntity==null){
...
//                            kebiaoEntity.setKebiaoName(data.get(0));                    //标题 要改的
//                            kebiaoEntity.setBanjiTypes(Integer.valueOf(data.get(0)));   //班级 要改的
//                            kebiaoEntity.setKebiaoPhoto("");//照片
//                            kebiaoEntity.setKebiaoContent("");//照片
//                            kebiaoEntity.setCreateTime(date);//时间
    ...
    */
    @RequestMapping("/add")
    public R add(@RequestBody KebiaoEntity kebiao, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,kebiao:{}",this.getClass().getName(),kebiao.toString());
        Wrapper<KebiaoEntity> queryWrapper = new EntityWrapper<KebiaoEntity>()
            .eq("banji_types", kebiao.getBanjiTypes())
            ;
        logger.info("sql语句:"+queryWrapper.getSqlSegment());
